...
<main>
    ...
    <div class="container border rounded-3 p-4 f1">
        <h2 class="mb-4"><span class="f1">รายการลูกค้า</span></h2>
        <table id="myTable" class="table table-striped table-hover f1">
            <thead class="table-success f1">
                <tr class="f1">
                    ...
                    <th class="f1">ชื่อ-นามสกุล</th>
                    <th class="f1">ที่อยู่1</th>
					<th class="f1">ที่อยู่2</th>
					<th class="f1">ที่อยู่3</th>
                    <th class="f1">เบอร์โทรศัพท์</th>
                    <th class="f1">อีเมล</th>
                    <th class="f1">รหัสผ่าน</th>
                </tr>
            </thead>
            <tbody>
<?php
...
if ($search) {
    $sql .= " WHERE c_fullname LIKE '%" . mysqli_real_escape_string($conn, $search) . "%'";
}
...
?>
                <tr class="f1">
                    <td class="f1"><a href="a-updatecustomer.php?cid=<?=$data['c_id'];?>" class="btn btn-warning btn-sm f1"><i class="bi bi-pencil"></i>แก้ไข</a></td>
                    <td class="f1"><a href="delete.php?cid=<?=$data['c_id'];?>" onClick="return confirm('ยืนยันการลบ');" class="btn btn-danger btn-sm f1"><i class="bi bi-trash"></i> ลบ</a></td>
                    <td class="f1"><?= htmlspecialchars($data['c_id']); ?></td>
                    <td class="f1"><?= htmlspecialchars($data['c_fullname']); ?></td>
                    <td class="f1"><?= htmlspecialchars($data['c_address1']); ?></td>
					<td class="f1"><?= htmlspecialchars($data['c_address2']); ?></td>
					<td class="f1"><?= htmlspecialchars($data['c_address3']); ?></td>
                    <td class="f1"><?= htmlspecialchars($data['c_phonnumber']); ?></td>
                    <td class="f1"><?= htmlspecialchars($data['c_email']); ?></td>
                    <td class="f1"><?= htmlspecialchars($data['c_password']); ?></td>
                </tr>
<?php
}
?>
